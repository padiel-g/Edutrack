...
import { Eye, Plus, Search, Trash2, UserCheck, UserX } from "lucide-react";
...
export function TeachersTable() {
  ...
  async function deleteTeacher(teacher: Teacher) {
    if (!window.confirm(`Delete ${teacher.name}'s teacher account? This only works when the teacher has no timetable, attendance, result, assignment, material, register, or report records.`)) return;
    setUpdatingId(teacher.id);
    setError("");
    try {
      ...
      setError(err instanceof Error ? err.message : "Unable to delete teacher account");
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-semibold">Teachers</h2><p className="text-sm text-slate-500">Accounts created by administrators. Delete is available only when no records exist.</p></div>
